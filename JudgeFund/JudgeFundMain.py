...
class JudgeFundMain:
    ...
    def get_manager_info(self, company, df, target_labe, manager_name):
        ...
        target_labe_list=[]
        for index_code in target_df.index.tolist():
            if target_df.loc[index_code]['基金全称'].find('量化')!=-1:
                target_labe_list.append(index_code)
        target_labe_df = target_df.loc[target_labe_list]
        self.logger.info('基金经理%s,查询到其管理指数增强型产品%s只(含A/C类),'%(manager_name,target_df.shape[0]))
        if target_labe_df.empty:
            self.logger.info('但未找到其管理过的%s产品,需对其该方面投资经验保持谨慎！')
            return

        self.logger.info('其中%s产品共%s只,反映了该基金经理有一定相关产品的投资经验；'%(target_labe,target_labe_df.shape[0]))
        last_se = target_labe_df[target_labe_df['基金成立日']==target_labe_df['基金成立日'].min()].iloc[0]
    ...
    def get_target_label_fund(self, company_fund_df, target_labe='量化'):
        name_list = company_fund_df['基金全称'].tolist()
        num_list = [fund_f_num for fund_f_num in range(len(name_list)) if name_list[fund_f_num].find(target_labe) != -1]
        ...
        self.logger.info("其中，%s(%s)，基金经理%s,于%s成立，为该管理人旗下成立最早的%s产品，最新规模为%s亿元；" % (
            min_fund_esta_se['证券简称'], min_fund_esta_se.name, min_fund_esta_se['基金经理(现任)'], min_fund_esta_se['基金成立日'],
            target_labe, min_fund_esta_se['基金规模亿元']))
    ...
    def get_comyany_info(self, company, df, target_labe=''):
        ...
        if target_labe:
            ...
            for company_name, temp_df in df.groupby(by='基金管理人'):
                ...
                temp_name_list = temp_df['基金全称'].tolist()
                for fund_name in temp_name_list:
                    if fund_name.find(target_labe) != -1:
                        dic_target[company_name] = dic_target[company_name] + 1
            label_se = pd.Series(dic_target, name=target_labe + '产品数量')
            self.logger.info("有%s产品(含A/C类)的管理人共%s家" % (target_labe, len(label_se[label_se > 0])))
            per_rate_label = (list(label_se.unique()).index(label_se[company]) + 1) / len(list(label_se.unique()))
            per_rate_label_str = str(np.round(per_rate_label * 100, 2)) + '%'
            self.logger.info(
                '其中%s旗下%s产品共%s只，占所有管理人同类型数量的%s分位数' % (company, target_labe, label_se[company], per_rate_label_str))
            self.get_target_label_fund(company_fund_df, target_labe)
        return

    def get_main(self, company='万家基金管理有限公司', manager_name='乔亮'):
        df = pd.read_excel('指数增强评价指标.xlsx', index_col=0)
        self.get_comyany_info(company, df, target_labe='量化')
        self.get_manager_info(company, df, target_labe='量化', manager_name=manager_name)


if __name__ == '__main__':
    JudgeFundMainDemo = JudgeFundMain()
    JudgeFundMainDemo.get_main()
