...
class JudgeFundDC:
    def __init__(self):
        ...
        self.target_code = '519062.OF'

    def get_comyany_info(self, company, df, target_labe=''):
        self.logger.info("截止当前最新日期%s,现有量化对冲型基金(含A/C类)共%s只。" % (self.last_date_str, df.shape[0]))
        ...
        self.logger.info('%s旗下现有量化型产品共%s只(含A类C类),占所有管理人所持数量的%s分位数。' % (company, num_se[company], per_rate_str))
        if per_rate >= 0.7:
            self.logger.info("占比靠前，反映管理人在发行量化对冲型基金上的优秀运作能力。")
        elif 0.7 >= per_rate > 0.4:
            self.logger.info("占比中等，管理人发行量化对冲型基金的数量一般。")
        elif per_rate <= 0.4:
            self.logger.info('占比下游，管理人对量化对冲型基金发行数量较少。')
        ...
        self.logger.info("按各管理人发行最早量化对冲型基金的时间看，该产品发行时间占各管理人同类型的%s分位数"%esta_percent_str)
        if esta_percent>=0.6:
            self.logger.info("发行时间早与多数管理人，一定程度上反应了管理人更丰富的投资管理经验。")
        elif 0.3<=esta_percent<0.6:
            self.logger.info("发行时间排名中等，管理人整体投资管理经验中等水平")
        else:
            self.logger.info("发行时间较晚，需谨慎对待管理人可能对量化对冲型基金投资管理经验较短的问题")
    ...
    def get_manager_info(self, company, df, manager_name,target_labe=''):
        ...
        if target_df.empty:
            self.logger.info('未查询到基金经理%s管理过的量化对冲型产品，对其投资经验需保持谨慎！')
            return
        self.logger.info('基金经理%s,查询到其管理量化对冲型产品%s只(含A/C类),'%(manager_name,target_df.shape[0]))
    ...
    def get_main(self):
        df = pd.read_excel("量化对冲公募基金.xlsx", index_col=0)
        self.get_comyany_info(company='海富通基金管理有限公司',df=df)
        self.get_manager_info(company='海富通基金管理有限公司',df=df,manager_name='朱斌全')

if __name__=='__main__':
    JudgeFundDCDemo = JudgeFundDC()
    JudgeFundDCDemo.get_main()